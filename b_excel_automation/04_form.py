...
class ClassificationExcel:
    ...
    def __init__(self, order_xlsx_filename, partner_info_xlsx_filename):
        ...
        self.now_today = datetime.now().strftime('%Y-%m-%d')  # 일자
    ...
    def set_excel_form(self, file_name):
        ...
        ws.insert_rows(1)
        ws.insert_rows(2)
        ws['A1'].value = f'발송요청내역 [총 {m_row}건] {self.now_today}'  # 전체 행 개수를 세서 num 변수에 삽입
        ws['A1'].font = Font(size=11, bold=True)  # 1행 폰트 크기 11, bold
        # # A:U열 설정
        ...
        for row in ws.iter_rows(min_row=4):
            for cell in row:
                if cell.row != 1 and cell.row != 2 and cell.row != 3:  # 1,2,3행 제외
